...
const SHOULD_FETCH_SEED_AUDIO = process.env.SEED_FETCH_AUDIO === "true";
...
async function syncWordSeedToWordList(input: {
  ...
  for (const item of input.words) {
    const audioUrl = await resolveSeedAudioUrl(item.word);
...
async function resolveSeedAudioUrl(word: string) {
  if (!SHOULD_FETCH_SEED_AUDIO) {
    return null;
  }

  return fetchPronunciationAudioUrl(word);
}
